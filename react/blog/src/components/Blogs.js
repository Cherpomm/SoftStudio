...
const Blogs = ({ data }) => {
  ...
  const increment = () => {
    ...
    if (vote >= 3) return alert("Maximum upvote !!");
    // Start vote to 1
  ...
  return (
    <div
      id={data.name || ""}
      className="border-b border-gray-200 flex flex-row p-6 m-4 mx-52 shadow bg-white rounded-lg"
      style={{ height: "calc(35vh - 2rem" }}
    >
      <div className="flex flex-col items-center text-xl py-4 pr-4 border-gray-200 border-r">
        <a className="border-b border-gray-200 mb-4 pb-4" href={data.ig} target="_blank" rel="noreferrer">
          <figure>
            <img
              src={data.pictureUrl}
              alt={data.pictureUrl}
